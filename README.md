...
1. `python run_game.py --game dollar_auction --mode cheap` 预演 1–3 局，确认流程；
2. `--mode official` 正式跑局（可多跑几局备选，单局约 $3–8）；
3. 浏览器打开 `viewer/index.html`，把 `out/` 里的 `.jsonl` 拖进去；
4. 点播放（自动导演：高光自动放慢放大），用 OBS 录屏；
5. 剪映导入视频 + `*_speech.srt` / `*_thought.srt` 双轨字幕 → 文字转语音 → 加音乐 → 导出。
...
1. 复制 `games/dollar_auction/` 为 `games/<新游戏>/`；
2. 实现接口契约 5 函数：`initial_state` / `active_players` / `visible_state` / `get_prompt` / `apply` / `is_terminal`（外加 `legal_actions` / `display_state` / `game_meta` / `settlement`）；
3. 在 `config.yaml` 里选阵容和规则参数，`game_meta()` 里选盘面模板（leaderboard / proposal_vote / round_table / card_list）；
...
缺哪个 Key，对应班底成员就在 config.yaml 的 `cast` 里暂时移除即可。
